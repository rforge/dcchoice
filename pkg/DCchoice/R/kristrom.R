...
#   # if(!is.data.frame(data)) stop("the data must be stored in a data.frame")
#   
...
#   lhs <- formula[[2]]
#   rhs <- formula[[3]]
#   R <- eval(lhs, data)          # yes/no to the first stage bids
#   bid <- eval(rhs, data)          # yes/no to the second stage bids
# 
#   nobs <- length(R)
# 
#   data.table <- as.data.frame(cbind(bid, R))
# 
#   # making table
#   tab.dat <- table(data.table)
#   tab.dat <- cbind(tab.dat, rowSums(tab.dat))
#   tab.dat <- cbind(tab.dat, round(tab.dat[,2]/tab.dat[,3], 3))
#   tab.dat <- tab.dat[, -1]
#   colnames(tab.dat) <- c("yes", "total", "yes/total")
# 
...
#   p <- tab.dat[,3]
#   adj.P <- p
#   while(!identical(adj.P, sort(adj.P, decreasing = TRUE))){
#     for(i in 1:(M-1)){
#       if(adj.P[i] < adj.P[i+1]){
#         adj.P[i] <- adj.P[i+1] <- (tab.dat[i, 1] + tab.dat[i+1, 1])/(tab.dat[i, 2] + tab.dat[i+1, 2])
#       }
#     }
#   }
#   
